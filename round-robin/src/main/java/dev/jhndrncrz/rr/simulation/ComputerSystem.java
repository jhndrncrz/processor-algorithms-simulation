...
public class ComputerSystem {
  ...
  public void inputTimeSlice() {
    ...
    while (true) {
      try {
        Terminal.clearLine();
        String timeSliceInput = IO.displayPrompt("Enter the", "time slice");
        timeSlice = Integer.parseInt(timeSliceInput);
        ...
        IO.displayError("Please enter a valid value for the time slice.");
        IO.displayInfo("Time slice must be a positive integer.");
    ...
    processor.setTimeSlice(timeSlice);
  }
  ...
  public void inputProcesses() {
    ...
    for (int i = 0; i < processesCount; ++i) {
      ...
      while (true) {
        try {
          Terminal.clearLine();
          String arrivalTimeInput = IO.displayPrompt("\tEnter the",
              IO.applyStyle(String.format("arrival time of P%d", i + 1), "fg-magenta"));
          ...
          IO.displayError(String.format("Please enter a valid value for the %s.",
              IO.applyStyle(String.format("arrival time of P%d", i + 1), "fg-magenta")));
          IO.displayInfo(
              String.format("%s must be a non-negative integer.", IO.applyStyle("Arrival time", "fg-magenta italic")));
          Terminal.cursorPreviousLine(3);
      ...
      while (true) {
        try {
          Terminal.clearLine();
          String burstTimeInput = IO.displayPrompt("\tEnter the",
              IO.applyStyle(String.format("burst time of P%d", i + 1), "fg-cyan"));
          ...
          IO.displayError(String.format("Please enter a valid value for the %s.",
              IO.applyStyle(String.format("burst time of P%d", i + 1), "fg-cyan")));
          IO.displayInfo(
              String.format("%s must be a non-negative integer.", IO.applyStyle("Burst time", "fg-cyan italic")));
          Terminal.cursorPreviousLine(3);
  ...
  public void showProcessesList() {
    ...
    for (Process process : processes) {
      IO.display(
          String.format("| %12s | %10s | %8s | %17s |", process.getName(), process.getArrivalTime(),
              process.getBurstTime(), process.getRemainingTimesString()));
    }
  ...
  public void simulateProcessScheduling() {
    ...
    for (int i = 0; i < this.timelineProcesses.size(); ++i) {
      ...
      IO.display(
          "+==============+============+==========+=============+===================+===============+===============+============+");
      IO.display(
          "| Process Name | Arrival T. | Burst T. | Response T. |      Remaining T. | Completion T. | Turnaround T. | Waiting T. |");
      IO.display(
          "+==============+============+==========+=============+===================+===============+===============+============+");
      ...
      for (int j = 0; j < processes.size(); ++j) {
        ...
        if (process.getName().equals(this.timelineProcesses.get(i).getName())) {
          ...
          if (process.getInterruptCount() == process.getInterruptRetrievalCount()) {
            IO.display("|" + IO.applyStyle(String.format(" %12s ", process.getName()), "fg-green")
                + String.format("| %10s | %8s | %11s | %17s | %13s | %13s | %10s ", process.getArrivalTime(),
                    process.getBurstTime(), process.getResponseTime(),
                    process.getRemainingTimesString(process.getInterruptRetrievalCount()), "--", "--", "--")
                + "|");
          } else {
            IO.display("|" + IO.applyStyle(String.format(" %12s ", process.getName()), "fg-green")
                + String.format("| %10s | %8s | %11s | %17s | %13s | %13s | %10s ", process.getArrivalTime(),
                    process.getBurstTime(), process.getResponseTime(),
                    process.getRemainingTimesString(process.getInterruptRetrievalCount()), process.getCompletionTime(),
                    process.getTurnaroundTime(), process.getWaitingTime())
                + "|");
          }
        } else if (this.timelineProcesses.subList(0, i + 1).stream().map(Process::getName).toList()
            .contains(process.getName())) {
          if (process.getInterruptCount() == process.getInterruptRetrievalCount()) {
            IO.display(String.format("| %12s | %10s | %8s | %11s | %17s | %13s | %13s | %10s |", process.getName(),
      ...
      for (int j = 0; j < this.timelineProcesses.size(); ++j) {
        if (j < i) {
          ...
          timelineLabel
              .append(IO.applyStyle(String.format(" %5s ", this.timelineProcesses.get(j).getName()), "fg-green"));
        } else {
  ...
  public void summarizeProcessSchedule() {
    ...
    for (Process process : outgoingProcesses) {
      ...
      IO.display(String.format("| %12s | %10s | %8s | %11s | %17s | %13s | %13s | %10s |", process.getName(),
          process.getArrivalTime(),
          process.getBurstTime(), process.getResponseTime(), process.getRemainingTimesString(),
